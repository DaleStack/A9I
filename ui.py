...
from PyQt6.QtCore import QEasingCurve, QPropertyAnimation, Qt, QTimer, QPoint, pyqtSignal, QObject, QEvent
...
class A9IFrame(QWidget):
    ...
    def changeEvent(self, event):
        """Fires whenever the window's state changes."""
        if event.type() == QEvent.Type.ActivationChange:
            # If the window just lost OS focus (user clicked somewhere else)
            if not self.isActiveWindow():
                # Ensure we don't accidentally close if they are just scrolling the dropdown list
                if self.combo.isVisible() and self.combo.view().isVisible():
                    return
                # Trigger dismissal
                self.start_fade_out()
    ...
    def show_selection_ui(self, mode, languages, text, pos):
        ...
        # Grab Focus & Auto-Open 
        self.show()
        self.activateWindow() # Forces OS to look at A9I so it can detect when you click away
        self.combo.setFocus()
        self.combo.showPopup() # Auto-drops the list down to save you a click!
    ...
    def show_translation(self, text, pos):
        ...
        self.label.setStyleSheet("QLabel { background-color: rgba(20,20,20,240); color: #00FF99; border: 1px solid #333; border-radius: 12px; padding: 15px; }")
    ...
    def mousePressEvent(self, event):
        """Clicking directly on the bubble still dismisses it."""
